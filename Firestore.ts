...
class Firestore implements FirestoreRead, FirestoreWrite, FirestoreDelete {
   ...
   */
  constructor(email: string, key: string, projectId: string, apiVersion: Version = 'v1', databaseId = '(default)') {
 ...
 */
function getFirestore(email: string, key: string, projectId: string, apiVersion: Version = 'v1', databaseId = '(default)'): Firestore {
  return new Firestore(email, key, projectId, apiVersion, databaseId);
}
